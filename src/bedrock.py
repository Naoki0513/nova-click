...
def call_bedrock_api(bedrock_runtime, messages: List[Dict[str, Any]], 
                    system_prompt: str, model_id: str, tool_config: Dict[str, Any]) -> Dict[str, Any]:
...
def analyze_stop_reason(stop_reason: str) -> Dict[str, Any]:
    ...
    if stop_reason == "end_turn":
        ...
        return {"should_continue": False, "error": True, "message": "LLMがtool_useで停止しましたが、toolUseブロックがありませんでした。"}
    elif stop_reason == "max_tokens":
        add_debug_log("Stop reasonが 'max_tokens' のため終了します。", level="WARNING")
        return {"should_continue": False, "error": False, "message": "最大トークン数に達したため、応答が途中で打ち切られている可能性があります。"}
    elif stop_reason:  # 他のstop_reason
        add_debug_log(f"Stop reason '{stop_reason}' のため終了します。")
        return {"should_continue": False, "error": False, "message": f"停止理由: {stop_reason}"}
    else:  # stop_reason が null や空文字の場合 (通常は発生しないはず)
        add_debug_log("Stop reasonが不明です。予期せぬ状態のためループを終了します。")
        return {"should_continue": False, "error": True, "message": "LLMが予期せぬ状態で停止しました（Stop reason不明）。"}
...
def create_bedrock_client(credentials: Dict[str, Any]) -> Any:
