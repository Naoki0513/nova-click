...
def test_normal_case(url="https://www.google.co.jp/", ref_id=13, text="Amazon", operation_timeout=10):
    ...
    if (search_input_ref_id is None and
        ...
        for test_ref_id in [17, 18, 20, 21, 22, 23]:  # ref_id=19はsubmitボタンなので除外
            ...
            matching_elem = next((e for e in elements_before if e.get('ref_id') == test_ref_id), None)
            if matching_elem:
    ...
    # 対象要素の情報を記録
    target_element = next((e for e in elements_before if e.get('ref_id') == actual_ref_id), None)
    if target_element:
        logging.info("入力対象要素: ref_id=%s, role=%s, name=%s",
    ...
    if aria_after_res.get("status") == "success":
        ...
        if len(elements_after) != len(elements_before):
            logging.info("DOM要素数が変化しました: %d → %d", len(elements_before), len(elements_after))
        else:
            logging.info("DOM要素数は変化していません: %d", len(elements_after))
        
        # Google検索結果ページの特徴を確認
        search_results = [e for e in elements_after if 
                         e.get('role') in ['heading', 'link'] and 
                         text.lower() in str(e.get('name', '')).lower()]
        if search_results:
            logging.info("検索結果ページと思われる要素を %d 個発見しました", len(search_results))
            for i, result in enumerate(search_results[:3]):  # 最初の3つだけログ出力
                logging.info("検索結果 #%d: role=%s, name=%s", 
                           i+1, result.get('role'), result.get('name'))
...
def test_error_case(url="https://www.google.co.jp/", ref_id=9999, text="Amazon", operation_timeout=10):
    ...
    if snap_before_res.get("status") != "success":
        logging.error("操作前のARIA Snapshot取得に失敗: %s", snap_before_res.get('message'))
        assert False, "ARIA Snapshot取得に失敗"
    ...
    if input_res.get("status") == "error":
        ...
        if current_url_res.get("status") == "success":
            ...
            if current_url == initial_url:
                ...
                logging.warning("エラー発生にもかかわらずURLが変化しています: %s → %s", 
        ...
        if snap_after_res.get("status") == "success":
            ...
            if len(elements_after) == len(elements_before):
                ...
                logging.warning("エラー発生にもかかわらず要素数が変化しています: %d → %d", 
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[0].endswith('__main__.py'):
        ...
        for arg in sys.argv[1:]:
            if arg in ['--debug', '--url', '--ref-id', '--text', '--timeout'] or not arg.startswith('-'):
