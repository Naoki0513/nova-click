...
def test_normal_case(url="https://www.google.co.jp/", ref_id=26):
    ...
    if aria_before_res.get("status") != "success":
        logging.error("クリック前のARIA Snapshot取得に失敗: %s", aria_before_res.get('message'))
    ...
    if not element_exists:
        logging.error("指定されたref_id=%s の要素が見つかりません。クリック可能な要素を探します。", ref_id)
        ...
        for elem in elements_before:
            if elem.get('role') in ['button', 'link']:
                ...
                logging.info("クリック可能な要素を見つけました: ref_id=%s, role=%s, name=%s",
        ...
        if clickable_elements:
            ...
            if elements_before:
                ...
                logging.info("テスト継続のため、最初の要素 ref_id=%s を使用します", ref_id)
            else:
                assert False, "クリック可能な要素が見つかりません"

    # 選択した要素の情報をログ
    target_element = next((e for e in elements_before if e.get('ref_id') == ref_id), None)
    if target_element:
        logging.info("クリック対象要素: ref_id=%s, role=%s, name=%s",
                    target_element.get('ref_id'), target_element.get('role'), target_element.get('name'))
    ...
    if aria_after_res.get("status") != "success":
        logging.error("クリック後のARIA Snapshot取得に失敗: %s", aria_after_res.get('message'))
    ...
    if len(elements_before) != len(elements_after):
        has_change = True
        logging.info("要素数の変化を検出: 前=%d, 後=%d", len(elements_before), len(elements_after))
    
    # 2. リンク要素の場合はURLの変化を検証
    if target_element and target_element.get('role') == 'link':
        current_url_res = goto_url("")  # 現在のURLを取得
        if current_url_res.get("status") == "success" and current_url_res.get("current_url") != url:
            has_change = True
            logging.info("URLの変化を検出: %s → %s", url, current_url_res.get("current_url"))
    
    # 検証結果
    if has_change:
        logging.info("クリック操作による変化を確認しました")
    else:
        logging.warning("クリック操作後の変化が検出できませんでした。実際の操作は成功している可能性があります。")
...
def test_error_case(url="https://www.google.co.jp/", ref_id=9999):
    ...
    if click_res.get("status") == "error":
        ...
        # 要素数が変わっていないことを確認
        if len(elements_before) == len(elements_after):
            logging.info("エラー後も要素数に変化がないことを確認: %d", len(elements_after))
        else:
            logging.warning("エラー後に要素数が変化しています: 前=%d, 後=%d", 
                          len(elements_before), len(elements_after))
